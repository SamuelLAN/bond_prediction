...
model_log_path = r'D:\Github\bond_prediction_branches\dev_2.0\bond_prediction\runtime\model_for_dealer_prediction.log'
...
# group_json_path = os.path.join(root_dir, 'groups_dealer_prediction', f'{choose_group}.json')
group_json_path = os.path.join(root_dir, 'groups_dealer_prediction', f'group_k_means_cluster_4_feat_1_trace_count_2_volume_3_num_dealer_split_by_date.json')
...
def parse_one_model_log(content):
    ...
    for k, v in content:
        ...
        if not is_individual_result:
            ...
            tmp_bond_id = k.replace('.pkl', '')
            tmp_i_group_index = group_json[tmp_bond_id]
            data['bond_results'].append(
                np.array([tmp_bond_id, tmp_i_group_index,
                          [tmp_result['precision'], tmp_result['recall'], tmp_result['f1']]]))

    result = data['test_result_dict']

    data['is_individual'] = False if data['bond_results'] else True
    data['is_all'] = True if not data['is_individual'] and data['data_param']['group_file_name'] == 'all' else False
    data['bond_id'] = '' if data['bond_results'] else data['data_param']['group_file_name'][1:]
    data['group_index'] = data['data_param']['group_file_name'].split('_')[1] \
        if not data['is_individual'] and not data['is_all'] else ''
    data['i_group_index'] = group_json[data['bond_id']] if data['is_individual'] else ''
...
def parse_model_log(_path, total_dict, group_name):
    ...
    for content in content_list:
        ...
        bond_id = tmp_data['bond_id']
        group_index = tmp_data['group_index']
        i_group_index = tmp_data['i_group_index']
        p_r_f1 = tmp_data['p_r_f1']
        bond_results = tmp_data['bond_results']

        if is_individual:
            total_dict[_model_name]['individual_results'].append([bond_id, i_group_index, p_r_f1])

        elif is_all:
            total_dict[_model_name]['all_results'] = bond_results
            total_dict[_model_name]['p_r_f1']['all'] = p_r_f1

        else:
            total_dict[_model_name]['group_results'] += bond_results
...
def convert_2_csv(total_dict, _path):
    ...
    results_order = ['g_g3', 'g_g1', 'g_g0', 'g_g2']
...
convert_2_csv(summary, os.path.join(root_dir, 'runtime', 'exp_results_dealer_prediction_1.csv'))
...
for i, lines in enumerate(content):
    ...
    _bond_results = lines[11:]
    _bond_results = list(map(lambda x: x.strip().split(': {'), _bond_results))
    ...
    _results = list(map(lambda x: [x[0].strip('.pkl'), json.loads('{' + x[1].replace("'", '"'))], _bond_results))

    for bond_id, _result in _results:
        if bond_id not in data:
            data[bond_id] = {}
        data[bond_id][group_index] = _result['f1']
...
for i, lines in enumerate(content_individual):
    ...
    bond_id = str(json.loads(lines[1][len('data_param: '):].strip())['group_file_name'][1:])

    # add data record to data dict
    if bond_id not in data:
        data[bond_id] = {}
    data[bond_id]['transformer2_individual'] = result['f1']
...
#         bond_id = group_file_name.split('.')[0]
# 
#         if bond_id not in data:
#             data[bond_id] = {}
#         data[bond_id][group_index] = result['f1']

# for bond_id, f1 in data.items():
#     print(bond_id, f1)

new_data_first = ['bond_id', 'transaction_count', 'transformer2_all', 'transformer2_individual', 'transformer2_k_means', 'transformer2_sc_trace_cluster_6', 'transformer2_sc_trace', 'k_means_group_index', 'sc_trace_cluster_6_group_index', 'sc_trace_group_index']
new_data = []
for bond_id, f1_dict in data.items():
    if 'transformer2_individual' not in f1_dict:
        f1_dict['transformer2_individual'] = ''

    # get transaction count
    transaction_count = len(train_d_dealers[bond_id]) if bond_id in train_d_dealers \
        else len(test_d_dealers[bond_id])
    ...
    # integrate results
    tmp_list = [bond_id, transaction_count] + f1_list + group_indexs
