...
class Loader:
    ...
    def __init__(self, input_days=20, force_refresh=False):
        ...
        data_all_root_dir = os.path.join(data_subset, year)
        all_level = os.path.split(data_all_root_dir)[1]

        self.__data_pkl_path = os.path.join(
            path.PATH_TMP_DIR,
            f'temporal_input_interval_output_with_all_average_same_dim_{all_level}_{subset}_{year}_{volume_level}_{data_index}_no_below_{no_below}_input_days_{input_days}.pkl')
        ...
        if os.path.isfile(self.__data_pkl_path) and not force_refresh:
            ...
            # load doc list
            train_doc_list, train_start_timestamp, train_end_time_stamp = self.__load_dir(
                os.path.join(data_root_dir, 'train'))
            test_doc_list, test_start_timestamp, test_end_time_stamp = self.__load_dir(
                os.path.join(data_root_dir, 'test'))

            data_all_pkl_path = os.path.join(path.PATH_TMP_DIR,
                                             f'all_doc_list_{subset}_{year}_{all_level}.pkl')
            ...
            if os.path.isfile(data_all_pkl_path):
                ...
                for _volume in os.listdir(data_all_root_dir):
                    ...
                    sub_train_all_doc_list = self.__load_dir_all(sub_all_root_dir, train_start_timestamp, train_end_time_stamp,
                                                             'train')
                    sub_test_all_doc_list = self.__load_dir_all(sub_all_root_dir, test_start_timestamp, test_end_time_stamp,
                                                            'test')
            ...
            #     train_all_docs += v
            # self.dict_all, self.voc_size_all = self.__gen_dict(train_all_docs, 200)
            ...
            self.__train_all_X = np.zeros(list(self.__train_X.shape[:-1]) + [self.voc_size], dtype=np.float32)
            self.__test_all_X = np.zeros(list(self.__test_X.shape[:-1]) + [self.voc_size], dtype=np.float32)

            for doc_list in train_all_doc_list:
                self.__train_all_X += self.__convert_input(doc_list, self.dict, self.voc_size, 'allow_unknown')
            self.__train_all_X /= len(train_all_doc_list)

            for doc_list in test_all_doc_list:
                self.__test_all_X += self.__convert_input(doc_list, self.dict, self.voc_size, 'allow_unknown')
            self.__test_all_X /= len(test_all_doc_list)
